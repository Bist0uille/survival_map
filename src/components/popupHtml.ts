...
export function poiPopupHtml(poi: Poi): string {
  const cat = getCategory(poi.categoryId)
  const rows = INTERESTING_TAGS.filter((k) => poi.tags[k])
    .map(
      (k) =>
        `<div class="text-slate-500"><b>${esc(k)}</b> : ${esc(poi.tags[k])}</div>`,
    )
    .join('')
  return `
    <div style="min-width:140px">
      <div style="color:${cat.color};font-weight:600">${esc(poi.name)}</div>
